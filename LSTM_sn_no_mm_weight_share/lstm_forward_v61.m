function lstm_forward_v61(in, label)
    global config mem;
    ...
    for L = 1:config.hidden_layer_num    % for each hidden layer
        for s = 1:config.slide_pieces
            ...
            for t = 1:size(in,2)    % for each time step
                ...
                mem.cell_in{L}{s}(:,:,t) = tanh(bsxfun(@plus, config.weights.Whg{L}{s} * hprev + cell_in_in(:,:,t), config.weights.Bg{L}{s}));
                mem.in_gate{L}{s}(:,:,t) = sigmoid(bsxfun(@plus, config.weights.Whi{L}{s} * hprev + in_gate_in(:,:,t), config.weights.Bi{L}{s}));
                mem.forget_gate{L}{s}(:,:,t) = sigmoid(bsxfun(@plus, config.weights.Whf{L}{s} * hprev + forget_gate_in(:,:,t), config.weights.Bf{L}{s}));
                mem.out_gate{L}{s}(:,:,t) = sigmoid(bsxfun(@plus, config.weights.Who{L}{s} * hprev + out_gate_in(:,:,t), config.weights.Bo{L}{s}));
    ...
    for s = 1:config.slide_pieces
        mem.net_out(:,size(in,2)-config.output_valid_len+1:end,:,s) = permute(softmax(pagefun(@mtimes, repmat(config.weights.Wy{s}, [1 1 config.output_valid_len]), mem.cell_acts{config.hidden_layer_num}{s}(:,:,size(in,2)-config.output_valid_len+1:end))), [1 3 2]);
