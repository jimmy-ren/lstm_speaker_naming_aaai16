function set_grad_to_zeros_v7()
    ...
    for L = 1:config.full_layer_num        
        mem.grad.Wfull{L} = config.NEW_MEM(zeros(size(config.weights.Wfull{L})));
        mem.grad.Bfull{L} = config.NEW_MEM(zeros(size(config.weights.Bfull{L})));
    end
