...
%     b_offset = 0.1;
    ...
    for s = 1:config.slide_pieces
        config.weights.Whg{1}{s} = config.NEW_MEM(rand(config.hidden_layer_size(1),config.hidden_layer_size(1))*2*r-r);
        config.weights.Whi{1}{s} = config.NEW_MEM(rand(config.hidden_layer_size(1),config.hidden_layer_size(1))*2*r-r);
        config.weights.Who{1}{s} = config.NEW_MEM(rand(config.hidden_layer_size(1),config.hidden_layer_size(1))*2*r-r);
        config.weights.Whf{1}{s} = config.NEW_MEM(rand(config.hidden_layer_size(1),config.hidden_layer_size(1))*2*r-r);        
...
%         config.weights.Bg{1}{s} = 0;
    ...
    for L = 1:config.hidden_layer_num
        for s = 1:config.slide_pieces
            mem.cell_in{L}{s} = config.NEW_MEM(zeros(size(config.weights.Whg{L}{s},1), config.batch_size, config.max_time_steps));
