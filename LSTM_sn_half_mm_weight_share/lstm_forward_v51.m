function lstm_forward_v51(in, label)
    global config mem;
    ...
    for L = 1:config.hidden_layer_num    % for each hidden layer
        for s = 1:config.slide_pieces
            x = get_x(L, s, in);
            cell_in_in = pagefun(@mtimes, repmat(config.weights.Wxg{L}{s}/1, [1 1 size(in,2)]), x);
            in_gate_in = pagefun(@mtimes, repmat(config.weights.Wxi{L}{s}/1, [1 1 size(in,2)]), x);
            forget_gate_in = pagefun(@mtimes, repmat(config.weights.Wxf{L}{s}/1, [1 1 size(in,2)]), x);
            out_gate_in = pagefun(@mtimes, repmat(config.weights.Wxo{L}{s}/1, [1 1 size(in,2)]), x);
